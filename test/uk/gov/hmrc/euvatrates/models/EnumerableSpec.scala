...
class EnumerableSpec extends AnyFreeSpec with Matchers with EitherValues with OptionValues with Enumerable.Implicits {
  ...
  ".reads" - {
    ...
    "must fail to bind for non-string types" in {
      val invalidJsons = Seq(JsNull, JsBoolean(false), JsNumber(123), JsArray(Seq()), JsObject(Seq()))

      invalidJsons.foreach { invalidJson =>
        Json.fromJson[Foo](invalidJson).asEither.left.value must contain(JsPath -> Seq(JsonValidationError("error.invalid")))
      }
    }
  }
